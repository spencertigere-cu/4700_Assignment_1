...
thermal_vel = sqrt((2*boltzmann_constant*T)/effective_mass);   % Thermal velocity. I lost marks here in my first submission attempt.
fprintf('Using a temperature of 300 K, the thermal velocity is %f km/s. \n', thermal_vel/1000)
tmn = 2e-13;% Mean time between collisions
freepath = thermal_vel*2e-13;%Mean free path. Tis calculation was missing in my first submission attempt.
...
E_xvel = thermal_vel.*cos(angle);
E_yvel = thermal_vel.*sin(angle);
...
xlabel('x-position (m)')
ylabel('y-position (m)')
title('Trajectory of Electrons in Random Motion')
figure(2)
plot(temp)
grid on
xlabel('Time')
